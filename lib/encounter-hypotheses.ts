 ...
 *
 * THE INTERVAL SHAPE mirrors lib/encounter-clock/smooth.ts's `Encounter` field for field. It is
 * declared here structurally rather than imported, because the encounter-clock branch has not landed
 * on this base; a smoother output object satisfies it as-is.
 *
...
export type HypothesisInterval = {
  ...
  identity?: { clinician_id: string | null; match_source: string; centroid_id: string | null; doctor_cosine: number } | null;
};
...
export function checkInterval(h: HypothesisInterval): RunProblem[] {
  ...
  const ok =
    ...
    (h.closed_by === "non_speech" || h.closed_by === "end_of_input") &&
  ...
  if (id) {
    const good =
      typeof id.match_source === "string" && VERSION_RE.test(id.match_source) &&
...
export function rowToHypothesis(r: Row): StoredHypothesis {
  const hasIdentity = r.match_source !== null && r.match_source !== undefined;
  return {
    id: String(r.id),
    ...
    closed_by: r.closed_by === "non_speech" ? "non_speech" : "end_of_input",
    ...
    identity: hasIdentity
      ? {
          ...
          match_source: String(r.match_source),
