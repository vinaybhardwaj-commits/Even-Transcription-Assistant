  ...
  switch (method) {
    case "initialize": {
      ...
      return rpcResult(id, {
        ...
        instructions:
          "Even Scribe operator door (S2): read tools over rooms, brain state/cues, Bench sessions/recordings, STT lab, voice, encounters, traces, stores, jobs and the audit log; plus room control through scribe_room_command — kind start_day/pause_day/resume_day/end_day goes to the room's listening kiosk (start_day is idempotent and refuses a consent-paused room unless override_pause), set_audio_input/check_update_now/report_diag/restart_engine go to the native Room Recorder, and close_orphaned_session is a server-side repair. Related tools are grouped behind one argument (view, aspect, source, action or kind); every tool name published before the grouping, scribe_start_recording included, is still accepted by tools/call. Defaults are summaries + pointers; pass include_payload / include_text / include_prompts / include_identity / include_urls explicitly.",
      });
    }
    case "ping":
      return rpcResult(id, {});
