 ...
 *   - listeners: [] with note "command bus not deployed (S2)".
...
export async function composeHealth() {
  const [app, brain, stt, pyannote] = await Promise.all([appHealth(), brainHealth(), sttHealthSafe(), probePyannote()]);
  ...
  return {
    ...
    listeners: [] as Array<{ room_id: string; last_poll_at: string; recording: boolean }>,
    note: "command bus not deployed (S2)",
...
const ENV_NAMES = [
  ...
  "OLLAMA_BASE_URL", "BRAIN_BASE_URL", "DEEPGRAM_API_KEY", "SARVAM_API_KEY", "ELEVENLABS_API_KEY",
...
const scribeSystemMap: McpTool = {
  ...
  handler: async () => {
    ...
    return {
      ...
      mcp: { path: "/api/mcp", token_env: "SCRIBE_MCP_TOKEN", slice: "S1 read-only tools", command_bus: "not deployed (S2)" },
    };
  },
};
