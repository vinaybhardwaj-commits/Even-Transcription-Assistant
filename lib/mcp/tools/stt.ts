...
const listSttRuns: McpTool = {
  ...
  description: "Encounters that have batch ASR runs (transcription_run mode=batch tier=asr): per-encounter engine count, errors, winner, gold flag, avg judge. Same as GET /api/admin/stt-lab/runs. patient_label only with include_identity=true.",
  scope: "read",
  inputSchema: {
    type: "object",
    properties: {
  ...
  handler: async (args: ToolArgs) =>
    failSafe({ runs: [] as unknown[] }, async () => {
      ...
      // Copied verbatim from app/api/admin/stt-lab/runs/route.ts GET.
      ...
      const runs = rows.map((r) => {
        ...
        return includeIdentity ? { ...rest, patient_label_raw } : rest;
      });
      return { runs };
    }),
};
...
const getSttRun: McpTool = {
  ...
  description: "One encounter's per-engine batch runs + scores + gold reference (transcription_run / stt_gold). Same as GET /api/admin/stt-lab/runs/{id}. Transcript / note / gold TEXT only with include_text=true; patient_label only with include_identity=true.",
  scope: "read",
  inputSchema: {
    type: "object",
    properties: {
  ...
  handler: async (args: ToolArgs) =>
    failSafe({ encounter: null as unknown, runs: [] as unknown[], gold: null as unknown }, async () => {
      const id = argStr(args, "encounter_id");
      if (!id || !id.startsWith("enc_")) return { encounter: null, runs: [], gold: null, error: "bad_encounter_id" };
      const includeText = argBool(args, "include_text");
      const includeIdentity = argBool(args, "include_identity");
      // Copied verbatim from app/api/admin/stt-lab/runs/[id]/route.ts GET.
      ...
      if (!enc[0]) return { encounter: null, runs: [], gold: null, error: "encounter_not_found" };
      ...
      const { patient_label_raw, ...encRest } = enc[0];
      ...
      return {
        encounter: includeIdentity ? enc[0] : encRest,
