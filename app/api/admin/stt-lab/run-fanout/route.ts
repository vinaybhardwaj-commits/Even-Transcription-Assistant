...
import { drainFanout, enqueueBackfill, fanoutStatus, resetAllJobs, dedupRuns, scribePending, runScribeForEncounter } from "@/lib/stt/fanout";
...
export async function POST(req: NextRequest) {
  ...
  let body: { limit?: number; backfill?: boolean; status?: boolean; reset?: boolean; score?: boolean; rescore?: boolean; dedup?: boolean; scribe?: boolean; encounterId?: string } = {};
  ...
  if (body.scribe) {
    ...
    const limit = Math.min(Math.max(Number(body.limit) || 3, 1), 10); return respondOk(await scribePending(limit));
  }
