 ...
 *
 * Body: { session_id, idx, content_type?, source? }   source (K-B): 'primary' (default —
 *       absent = today's behaviour) | 'backup' (second-mic stream, backup_chunk_{idx}.webm)
...
export async function POST(req: NextRequest) {
  ...
  let body: { session_id?: unknown; idx?: unknown; content_type?: unknown; source?: unknown };
  try {
  ...
  try {
    const existing = (await sql`
      SELECT upload_state FROM bench_chunk
       WHERE session_id = ${sessionId} AND source = ${source} AND idx = ${idx}
       LIMIT 1
