 ...
 * content — important because the log documents still-unpatched security issues.
 *
 * Content: a repo-embedded snapshot of ETA-BUG-LOG.md (content/buglog-content.ts,
 * regenerated by scripts/sync-buglog.mjs), rendered by the dependency-free
 * lib/markdown-min renderer (all HTML escaped).
 */
import type { Metadata } from "next";
import { readAdminCookie } from "@/lib/cookie";
import { verifyAdminJwt } from "@/lib/auth";
import { renderMarkdown } from "@/lib/markdown-min";
import { BUGLOG_MD, BUGLOG_UPDATED_AT } from "@/content/buglog-content";
...
export default async function BugLogPage() {
  ...
  const html = renderMarkdown(BUGLOG_MD);
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: CSS }} />
      <div className="buglog-wrap">
        <div className="buglog-bar">
          <h1>Evenscribe — Bug Log</h1>
          <span className="meta">snapshot updated {fmt(BUGLOG_UPDATED_AT)}</span>
        </div>
