...
- **unreleased** — close ffmpeg/helper `Pipe()` ends after each piece encode and poll (`FoundationPieceProcessRunner`, `runTool`, `runHelperVersion`) so a long-lived process cannot leak FDs until `ulimit -n` / EMFILE.
