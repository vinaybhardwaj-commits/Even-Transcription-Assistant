...
# every nanosecond of the span either backed by samples or explained by a gap_ns. It does NOT check the
...
# carry a tape.idx, this script fails exactly the four whose index arithmetic is damaged
# (c1-offset-not-twice-samples, c1-restart-odd-tail-untrimmed, c3-blank-line-inside-torn-log, c4-one-sample-lost)
# and passes the other 24, which violate conversion rules while still being real tapes with real audio in them.
# That is the intended scope: "is there a tape with sound in it", not "is every rule obeyed".
#
# Exit status: 0 only when a tape with sound in it exists. 1 otherwise, or on any structural failure.
set -uo pipefail
...
print(f"  monotonic span      {mono_span} ns = {dur(mono_span)}")
...
unaccounted_steps, total_unaccounted, explained = [], 0, 0
for i in range(1, len(ckpts)):
    (n_prev, a), (n_cur, b) = ckpts[i - 1], ckpts[i]
    if int(b["mono_ns"]) < int(a["mono_ns"]):
        inv.append(f"line {n_cur}: mono_ns went backwards along the checkpoint spine, "
                   f"{a['mono_ns']} -> {b['mono_ns']}")
    between = [r for n, r in discs if n_prev < n < n_cur]
    gap_here = sum(int(r["gap_ns"]) for r in between if r.get("gap_ns") is not None)
    step = int(b["mono_ns"]) - int(a["mono_ns"]) - (int(b.get("samples", 0)) - int(a.get("samples", 0))) * NS_PER_SAMPLE
    if step:
...
if marker_inversions:
    ...
    for n, kind, by in marker_inversions[:10]:
        print(f"      line {n}: {kind} stamped {by} ns earlier — it carries the instant of the event")
coverage = (audio_ns / mono_span * 100) if mono_span else float("nan")
print(f"  coverage            {coverage:.4f}%  of the monotonic span is actually on tape")
...
# ---- verdict ----------------------------------------------------------------------------------------------------
# "A tape with sound in it" is two claims, decided separately and stated separately.
...
print(f"                {s_last} samples = {dur(audio_ns)} of audio, coverage {coverage:.4f}% of the span")
