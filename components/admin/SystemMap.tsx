...
const PHASES: Array<{ phase: string; blurb: string; stages: Stage[] }> = [
  {
    ...
    stages: [
      ...
      { n: 3, title: "Live transcript (parallel + language router)", detail: "Every chunk fans out to ALL live engines at once. A continuous LANGUAGE ROUTER (Whisper LID + Sarvam + script, hysteresis, English default) decides the current language each window and routes the PRIMARY on-screen box to the best engine — English -> Deepgram; non-English -> Sarvam + the IndicConformer native-script box (shown only when truly non-English). No engine is ever restarted; the router only picks which parallel stream to display. None of this is the saved note — it's the on-screen preview.", tiers: ["client", "third", "mini"], tech: ["Deepgram", "Sarvam", "Whisper", "IndicConformer", "language router"] },
  ...
  },
  {
    ...
    stages: [
      ...
      { n: 8, title: "Batch translate (non-English)", detail: "The English-vs-Indic fork is a CORROBORATED, English-biased decision (decideEncounterLanguage: Whisper LID + Sarvam + actual script) — never Sarvam's lone label, which has mislabelled accented English as Bengali. Only when corroborated non-English: the whole conversation is re-translated full-file by Sarvam (Saaras) into clean English (more accurate than live; safety net if live under-captured). Marked translated=true so a resume never repeats it. Soft-fails back to the live transcript.", tiers: ["vercel", "third"], tech: ["decideEncounterLanguage", "Sarvam Saaras", "translated flag"] },
...
const LIVE_ENGINES: Array<{ name: string; tier: TierKey; role: string }> = [
  { name: "Deepgram (live WS)", tier: "third", role: "English real-time transcript; PRIMARY live box when the language router says English; short-lived minted token, optional auto-reconnect." },
...
const ALGOS: Array<{ name: string; detail: string }> = [
  ...
  { name: "Language routing & detection", detail: "corroborated, English-biased language decision (decideEncounterLanguage: Whisper LID + Sarvam + Indic script) drives the submit fork; a live router (advanceRouter) re-evaluates each window with hysteresis (no single-window flips, English default) and routes the primary live box to the best engine. Engines run in parallel, so it's selection not switching; display-only — the note's language is decided at submit." },
...
export function SystemMap() {
  return (
    <div className="sm-wrap">
      ...
      <footer className="sm-foot">Architecture overview · derived from the codebase (refreshed 14 Jun 2026 — adds language routing). Deeper detail: the bug log at <code>/buglog</code> and the PRDs. This page is a living map — update it as the system evolves.</footer>
    </div>
  );
}
