...
- **D1b is RULED (V, 24 Sep 2026, standing; Fable ruling 42).** V, verbatim: "There is no limit to what jev can read, just try and make sure it is de-identified when the person is not a doctor." So Jev may read any ETA text, but BEFORE text leaves our machines for Jev, identifiers of NON-DOCTORS are removed locally (patient and attender names, phone numbers, addresses, ID numbers Aadhaar/PAN/UHID/MRN, email, date of birth, exact age over 89; typed tags `[PATIENT_NAME]` `[PHONE]` `[ID]` `[ADDRESS]` `[DOB]`, plus `[EMAIL]` and `[AGE]` for the email and over-89 rules; doctors' names stay, from the EPI physician roster and the voiceprint roster). The de-id step today is the surgery-review tool (eta-lab `studies/2026-09-24-surgery-review-v2/deid.py`, local, pinned offline model) and it is PILOT-CLEARED ONLY, for English text in the surgery pilot: always run it with `--drop-nonlatin-orig`; its default ships Indic-script names and its fuzzy roster match keeps near-miss patient names, so it is NOT a general de-id step until eta-refuter-2's findings G1-G4 close (eta-lab `studies/2026-09-24-surgery-review-v2/REFUTER-2-DEID-VERDICT.md`). Production `lib/jev` does NOT have a de-id step yet, so no production Jev flag reads real text until it does (later train item). This replaces the earlier D1a/D1b split and the narrower Workstream-H-only clearance (ruling 35). The tools screen for credentials, not for patient data — that screen is human, and it is you.
