...
-- itself to it. Six facts have to come back off the Mac for that to be observable from the fleet
...
-- why `applyInstallPoll` COALESCEs the other five and deliberately does not COALESCE this one.
--
...
--
-- THE THREE `last_update_*` COLUMNS ARE A RECEIPT, not a status. The swap script writes
...
--
-- ADDITIVE. Six nullable-or-defaulted columns. No row is rewritten, no constraint is added, nothing
...
COMMENT ON COLUMN room_install.last_update_error IS
  'The reason line from update-result.json, naming what went wrong and the version it was going to. COALESCEd with last_update_result. NULL when the last attempt succeeded or none has been made.';

COMMENT ON COLUMN room_install.last_update_at IS
  'When the swap script recorded that outcome, as it wrote it into update-result.json. The Mac''s clock, not the server''s — it is the time the attempt ended, and the app that attempted it had already exited. COALESCEd with the other two.';
