...
## 6. room-recorder 0.1.22 — BUILT AND SIGNED, **NOT PUBLISHED. Release id: none.**
...
The upload and `POST /api/admin/releases` were NOT run, so no release row exists and no Mac can be offered 0.1.22. Reason: publishing to `test` puts 0.1.22 on Home Office and Room 4.1 while production runs the pre-Tier-1 server, which cannot read `clip_count`/`silence_ms`/`channel_locked` and whose `bench_command.kind` CHECK predates the three verbs. Signing is otherwise ready — identity `187DD424…8EDB` valid, keychain unlocked.
