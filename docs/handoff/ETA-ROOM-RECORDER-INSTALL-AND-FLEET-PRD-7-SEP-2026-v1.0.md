...
## 13. Build R3 addendum, 8 September 2026 — self-update

Appended verbatim from `ETA-INSTALL-AND-FLEET-PRD-BUILD-R3-ADDENDUM-8-SEP-2026.md` during the
Build R3 session of 9 September 2026, per that build's kickoff §5.9. **§13 supersedes §7 where
the two disagree, and they disagree about steps 7 to 10.** It amends §7 and §8.

R3-1 to R3-8 were ratified by V on 8 September 2026; R3-9 to R3-12 on 9 September, as written.
Two scope additions were ratified on 9 September and are recorded in §13.8 below.

*Provenance.* Written against production sha `46a7475`, migrations through `0077`, branch
`feat/room-recorder` tip `1193083`, stable release `0.1.7`. Every code fact in it was read out of
`origin/feat/room-recorder`.
...
### 13.8 Scope additions ratified 9 September 2026

Recorded here so §13 is the whole of the R3 contract and no reader has to hold the kickoff open.

1. **Mockup state C.** The update-failure sentence sits in the **App cell**, under the version that
   did not change, because the failure is a fact about the version. State D — the same sentence under
   Last seen — does not ship. States A, B and E ship as drawn.
2. **`disk_free_bytes` joins migration 0078 as a sixth column**, read from the volume holding the
   captures directory via `URLResourceKey.volumeAvailableCapacityForImportantUsageKey`, and the
   hard-coded `spare_device` query item is removed from every poll. The app has no spare-microphone
   concept, so that literal was a constant standing in for a measurement, which §5.5 forbids. Free
   disk renders in the Machine cell as a plain size: no column, no control, no colour, no threshold.
   Nothing enforces retention today at about 115 MB per recorded hour.
