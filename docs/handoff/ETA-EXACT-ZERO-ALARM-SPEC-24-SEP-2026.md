...
**Status: DRAFT. Nothing is deployed, nothing is wired, nothing can write.** Delivered as this spec, an inert module (`lib/exact-zero-alarm.ts`) and its tests
...
**These numbers are guesses until calibrated.** Standing rule: measure the constant first. Bus #1295 asks herdr-kit to run ONE read-only aggregate on the Mini (counts only): per room and IST hour
