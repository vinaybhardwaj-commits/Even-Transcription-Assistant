...
CAVEAT FROM eta-refuter #1979 (accepted): the level log begins 22-23 Sep, and every room had been moved onto a C270 WEBCAM by 22 Sep (r267/r280), so all 171 "healthy" hours are webcam-mic hours and the 10% threshold is calibrated to the C270's noise floor. Ruling 335 switches OPD 3, Cardiology and OPD 7 back to their TM20s after V's check; a different mic, self-noise and input gain could make a quiet TM20 room print peak 0.0000 more often (false positives this window cannot show), and it cannot yet show that E catches the TM20-mute signature itself (that should read a share of 1.0). So: E stays in SHADOW, the calibration is RE-RUN over the first clinic days after the switch-back, PER MIC TYPE (which the device-context columns of migration 0120 make possible once deployed), and one real episode or a deliberate mute test with V on site would confirm the mute case.

## 10. CORRECTION, 25 Sep (Fable r355 CLOSED; eta-refuter #2095/#2098; scribe #2103), the commit time stands — this section supersedes §9 wherever they disagree

1. OPD 4's C270 is PROVEN WORKING (Fable played a chime and speech on OPD 4's own Mac speaker at 09:36:25-09:36:50 and read tape.pcm directly: 1-second peaks 0.08-0.32 during the test, 0.000-0.04 either side). That morning's near-silence (71 % of rows under 0.001) was an EMPTY, CLOSED ROOM, not a device fault. Near-silence from a fault and near-silence from an empty closed room READ THE SAME on this measure.
2. THEREFORE §9's calibration is WITHDRAWN as a fault calibration. What the 7 days show is which rooms read near-silent for long stretches (eight rooms never did; OPD 4 did in 8 to 9 of 16 judged hours), NOT that those hours were faults. There is NO verified positive: eta-refuter has no occupancy source for 23 Sep 15-19, and 24 Sep 12-20 shows the same C270 reading normal ambient. §9's "the one known fault", "catches more of the real fault" and "0 false alarms in 171 healthy hours" are retracted; the false-alarm rate on an empty closed room is, on 25 Sep's evidence, HIGH.
3. THE LEVEL-LOG PEAK IS A DUTY-CYCLED READING (eta-refuter #2095): one 1.25 s RMS window about every 2.2 s, 10 to 20 times below the tape's sample peaks, and it logged exact 0 on rows while Fable's chime was playing. A single row, and a small per-hour share of "peak = 0" rows, is not evidence about the tape.
4. WHAT THE ALARM MAY DO: NOTHING PAGES ON NEAR-SILENCE ALONE (Fable). Option E, if built at all, is a CANDIDATE LIST (rooms with long near-silent stretches) whose members are VERIFIED by a person or a speaker self-test (as Fable did on OPD 4 through the Mac's own speaker; there is no such verb in the app today). lib/near-silence-alarm.ts (inert, nothing calls it) says this in its header and in every message it can produce. Thresholds stay unruled: the numbers ruling is NOT to be asked of Fable on this data.
5. A BIT-EXACT TM20-MUTE DETECTOR is a different measure: the tape-level rule of record (full chunk at 707-709 B/s, floor 134.2 h; r343), not the level log and not zero_ratio, because a quiet C270 room also reads zero_ratio 0.45-0.98 (the ring on the morning of 25 Sep).
6. THE 27 SEP SUMMARY owed to Fable becomes the per-room hourly near-silence table WITH occupancy beside it (visit rows or PQM called_at/completed_at, once the visits switch is back on and a doctor is known for the room), not a threshold calibration. The shadow log keeps recording both floors meanwhile.
