...
| # | item | state | why here |
|---|---|---|---|
| S1 | **The room-day output surface.** Room transcripts, turns, speakers and emotion drain into tables nothing displays. V asked where it all shows up in EvenScribe; today the answer is *nowhere*. | **NEXT. Not started.** | Everything below is unreadable without it. It is also the only item V has asked for twice. |
| S2 | **Voice thresholds set from evidence.** E20 (`5e5d054`) shipped the losing score so a threshold can be chosen; the 0.65 room / 0.70 encounter / 0.78 phone cosines are still **unratified guesses**. | blocked on drained + diarized windows accumulating | It gates S4 and J3 — both defer to `clinician_id`, which those thresholds decide. |
...
| **J0** | **English window text** — `jev_window_text` + job `jev-english`. | **NEW, 18 Sep. Not started. Unblocks J2.** | See below. |
