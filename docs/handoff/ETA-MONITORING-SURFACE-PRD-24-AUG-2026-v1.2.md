...
| **D33a** | **Amended 25 August, during Build 2.** D33 read strictly would refuse every spare recorded before Build 2, because none of those pieces carry measurements — so a window would stay on a microphone already proven dead and resolve to silence, discarding the only recording of those minutes to satisfy a word. **Corrected: once the main is proven dead, the spare is refused only when proven BAD.** Convicting the main still needs positive proof; refusing the spare still needs positive proof; unmeasured is neither. The reason is recorded per window and distinguishes `main_dead_spare_healthy` from `main_dead_spare_unmeasured`. Raised by the builder, ratified by V. |
