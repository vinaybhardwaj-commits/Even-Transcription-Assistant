...
const SITES: Array<{ file: string; counts: Record<string, number>; role: SiteClass; why: string }> = [
  ...
  { file: "lib/whisper.ts", counts: { transcribeWithWhisper: 1, inference: 1 }, role: "not_a_window_reader", why: "the client itself — it PRODUCES EMPTY_TRANSCRIPT; its base URL now comes from the pool (lib/service-pool.ts)" },
  { file: "lib/service-pool.ts", counts: { WHISPER_BASE_URL: 1 }, role: "not_a_window_reader", why: "REDUNDANCY-R1 — names the env vars of each service's endpoint list; it calls nothing itself, the client passes it the call" },
